import eel, json, os, minecraft_launcher_lib,subprocess,uuid,shutil
...
def getAvailbleVersions():
    ...
    with open('config/forge.json', 'w') as f: 
        json.dump(forge, f, ensure_ascii=False, indent=4) 
    with open('config/fabric.json', 'w') as f: 
...
@eel.expose
def fabricAdd():
    with open("config/fabric.json") as f:
        data = json.load(f)
        for i in range(len(data)):
...
@eel.expose
def forgeAdd():
    with open("config/forge.json") as f:
        data = json.load(f)
        for i in range(len(data)):
            eel.forgeAdd(data[i])
